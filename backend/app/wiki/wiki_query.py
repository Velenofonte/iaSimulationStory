...
from app.player.knowledge import knows
from app.wiki.wiki_overlay import (
    is_overlay_path,
...
def build_world_excerpt(
    ...
    *,
    ...
    if not text:
        return ""
    # Never leak occult chronicle into generic world excerpts.
    text = re.sub(
        r"(?ms)^#\s*Cronaca occulta\s*\n.*?(?=^#\s|\Z)",
        "",
        text,
    )
    text = re.sub(
        r"(?ms)^#\s*Eventi occulti\s*\n.*?(?=^#\s|\Z)",
        "",
        text,
    )
    ...
    prefer_place_now = bool(
        re.search(
            r"(?im)^#\s*(Eventi|Difesa|Descrizione|Ruolo)\s*$",
            text,
        )
    ) and not prefer_adventurer and not prefer_magic
    if prefer_adventurer or prefer_magic or prefer_place_now:
        sections = [s.strip() for s in re.split(r"(?m)(?=^# )", text) if s.strip()]
        if sections:
            priority: list[str] = []
            rest: list[str] = []
            place_priority_re = re.compile(
                r"(?i)^#\s*(Eventi|Difesa|Descrizione|Ruolo)\b"
            )
            for sec in sections:
                title = sec.split("\n", 1)[0]
                if prefer_place_now and place_priority_re.search(title):
                    priority.append(sec)
                elif (prefer_adventurer or prefer_magic) and _EXCERPT_PRIORITY_TITLE_RE.search(
                    title
                ):
                    priority.append(sec)
                else:
                    rest.append(sec)
            if priority:
                # For places, Eventi/Difesa first among priority titles.
                if prefer_place_now:
                    def _place_rank(sec: str) -> int:
                        t = sec.split("\n", 1)[0].lower()
                        if "eventi" in t:
                            return 0
                        if "difesa" in t:
...
class WikiQuery:
    ...
    def read_page(self, path: Path) -> tuple[dict, str]:
        try:
            ...
            # Broken YAML frontmatter must not crash a turn: fall back to raw body.
            ...
            if text.startswith("---"):
                ...
                if len(parts) >= 3:
                    body = parts[2].lstrip("\n")
            return {"id": path.stem, "name": path.stem, "type": "unknown"}, body
        ...
        if loc_keys:
            for key in loc_keys:
                ...
                if loc_path and loc_path.exists():
                    for linked in self.linked_entities(loc_path):
                        # Spontaneous linked cast/lore only if the PC already knows them.
                        # Player-named tokens are added below via extract_entities_from_text.
                        if mode != "scene" or knows(game_state, linked):
                            page_ids.append(linked)
                    break
