...
from app.wiki.character_sheet_normalize import (
...
class WikiWriter:
        ...
        for character_id, update in result.character_updates.items():
            ...
            if getattr(update, "spells_add", None):
                ...
                for line in update.spells_add:
                    ...
                    if name:
                        spell_map[name] = coerce_spell_record(name, desc)
                if spell_map:
    ...
    def _write_spellbook_lines(
        ...
        player_name: str,
        player_id: str,
        spells: Mapping[str, str | SpellRecord | Mapping[str, Any]],
    ) -> None:
        body = "# Spellbook\n"
        if spells:
            lines: list[str] = []
            for name, value in spells.items():
                rec = coerce_spell_record(str(name), value)
                lines.append(
                    format_spell_line(
                        str(name),
                        rec.get("description") or "",
                        output=rec.get("output") or "effect",  # type: ignore[arg-type]
                        manifest=rec.get("manifest") or "visible",  # type: ignore[arg-type]
                    )
                )
    ...
    def _extract_spells_from_character(self, character_id: str) -> dict[str, SpellRecord]:
        ...
        for section in ("Incantesimi noti", "Incantesimi", "Magie", "Magia"):
            for line in self.get_section(body, section):
                ...
                name = parsed["name"]
                if name:
                    out[name] = SpellRecord(
                        description=parsed["description"],
                        output=parsed["output"],  # type: ignore[typeddict-item]
                        manifest=parsed["manifest"],  # type: ignore[typeddict-item]
                    )
        for match in re.finditer(r"\[([^\[\]]{2,60})\]", body):
            name = match.group(1).strip()
            if not name:
                continue
            out.setdefault(name, coerce_spell_record(name, ""))
        return out
    ...
    def read_spellbook(
        ...
        *,
        ...
        """Return [{name, description, output, manifest}, ...] deduped by name."""
        pid = (character_id or self.player_id(player_name)).strip()
        if ensure:
        ...
        needs_repair = False
        for line in lines:
            # Legacy / incomplete if no tag suffix
            ...
            if prev is None:
                by_key[key] = parsed
                order.append(key)
            elif len(parsed.get("description") or "") > len(prev.get("description") or ""):
                by_key[key] = parsed
        spells = [by_key[k] for k in order]
        if len(lines) > len(spells):
            needs_repair = True
        if needs_repair and spells:
            body = "# Spellbook\n" + "\n".join(
                f"- {format_spell_line(s['name'], s.get('description') or '', output=s.get('output') or 'effect', manifest=s.get('manifest') or 'visible')}"  # type: ignore[arg-type]
                for s in spells
            ) + "\n"
            post.content = body
            path.write_text(frontmatter.dumps(post), encoding="utf-8")
    ...
    def track_spells_for_id(
        ...
        *,
        ...
        changed = False
        for spell_name, value in spells.items():
            key = spell_name.strip().lower()
            if not key:
                continue
            existing_rec = by_name.get(key)
            rec = coerce_spell_record(spell_name.strip(), value, existing=existing_rec)
            new_desc = (rec.get("description") or "").strip()
            if key in by_name:
                prev = by_name[key]
                prev_desc = (prev.get("description") or "").strip()
                # Existing spells: never overwrite user/heuristic tags from track_spells.
                # Tags change only via set_spell_tags (UI / explicit edit).
                if new_desc and len(new_desc) > len(prev_desc):
                    prev = dict(prev)
                    prev["description"] = new_desc
                    # Keep canonical display name if we have a nicer one
                    if spell_name.strip():
                        prev["name"] = spell_name.strip()
                    by_name[key] = prev
                    changed = True
                elif not prev_desc and new_desc:
                    prev = dict(prev)
                    prev["description"] = new_desc
                    by_name[key] = prev
                    changed = True
        ...
        unique_existing = len(
            {parse_spell_entry_line(x)["name"].lower() for x in existing if x.strip()}
        )
        if len(existing) > unique_existing:
            changed = True
        # Also rewrite if any existing line lacked tags
        if any("{" not in x or "}" not in x for x in existing):
            changed = True
        ...
        lines = [
            format_spell_line(
                by_name[k]["name"],
                by_name[k].get("description") or "",
                output=by_name[k].get("output") or "effect",  # type: ignore[arg-type]
                manifest=by_name[k].get("manifest") or "visible",  # type: ignore[arg-type]
            )
            for k in order
        ]
    ...
    def set_spell_tags(
        ...
        *,
        ...
        """Update output/manifest tags for named spells; return full spellbook."""
        name = player_name or player_id
        self.ensure_spellbook(name, character_id=player_id)
        path = self.spellbook_path(player_id)
        if not path.exists():
        ...
        for upd in updates:
            key = str(upd.get("name") or "").strip().lower()
            if not key or key not in by_name:
                continue
            entry = dict(by_name[key])
            out = str(upd.get("output") or "").strip().lower()
            man = str(upd.get("manifest") or "").strip().lower()
            if out in {"info", "effect"}:
                entry["output"] = out
            if man in {"visible", "subtle"}:
                entry["manifest"] = man
            by_name[key] = entry
    ...
    def write_chronicle(self, entries: list) -> None:
        """Write session chronicle to wiki/world/cronaca.md (public + occult sections)."""
        path = self.wiki_dir / "world" / "cronaca.md"
        path.parent.mkdir(parents=True, exist_ok=True)
        public_lines: list[str] = []
        occult_lines: list[str] = []
        for entry in entries:
            summary = str(getattr(entry, "summary", "") or "").strip()
            if not summary:
                continue
            reach = str(getattr(entry, "reach", "local") or "local")
            secret = bool(getattr(entry, "secret", False))
            eid = str(getattr(entry, "id", "") or "")
            day = getattr(entry, "day", None)
            prefix = f"[{reach}]"
            if day is not None:
                prefix = f"{prefix} G{day}"
            if eid:
                prefix = f"{prefix} ({eid})"
            bullet = f"- {prefix} {summary}"
            if secret or reach == "none":
                occult_lines.append(bullet)
            else:
                public_lines.append(bullet)
        ...
        if path.exists():
            ...
            meta = {
                "id": "cronaca",
                "name": "Cronaca del mondo",
                "type": "world",
                "tier": "minimal",
                "source": "runtime",
            }
            body = "# Descrizione\nMemoria pubblica degli eventi rilevanti della sessione.\n"

        body = self.set_section(
            body,
            "Cronaca",
            "\n".join(public_lines) if public_lines else "",
        )
        body = self.set_section(
            body,
            "Cronaca occulta",
            "\n".join(occult_lines) if occult_lines else "",
        )
        path.write_text(
            frontmatter.dumps(frontmatter.Post(body if body.endswith("\n") else body + "\n", **meta)),
            encoding="utf-8",
        )
        self._add_index("cronaca", "world/cronaca.md")

    def append_chronicle_lines(self, public: list[str], occult: list[str] | None = None) -> None:
        """Append bullets to existing chronicle sections."""
        path = self.wiki_dir / "world" / "cronaca.md"
        if not path.exists():
            self.write_chronicle([])
        post = frontmatter.load(path)
        body = post.content or ""
        existing_public = self.get_section(body, "Cronaca")
        existing_occult = self.get_section(body, "Cronaca occulta")
        for line in public:
            text = line.lstrip("- ").strip()
            if text and text not in existing_public:
                existing_public.append(text)
        for line in occult or []:
            text = line.lstrip("- ").strip()
            if text and text not in existing_occult:
                existing_occult.append(text)
        body = self.set_section(
            body, "Cronaca", "\n".join(f"- {x}" for x in existing_public)
        )
        body = self.set_section(
            body, "Cronaca occulta", "\n".join(f"- {x}" for x in existing_occult)
        )
        post.content = body
        path.write_text(frontmatter.dumps(post), encoding="utf-8")
