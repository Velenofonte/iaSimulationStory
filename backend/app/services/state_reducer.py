...
def apply_scene_delta(state: GameState, delta: SceneStateDelta) -> None:
    ...
    for location_id, update in delta.location_runtime.items():
        ...
        if update.events:
            ...
            # Location events may open situation threads (legacy string → fact).
...
def resolve_presence_for_location_change(
    *,
    ...
    """When the PG moves, present must be re-evaluated.

    - present provided → use it (full replacement at the new place)
    - present omitted + location changed → [] (fail-closed: no stale cast)
    - no location change → present unchanged (None = no-touch)
    """
    if not new_location:
        return list(present) if present is not None else None
    prev = normalize_place_id(previous_location or "") or (previous_location or "")
    nxt = normalize_place_id(new_location) or new_location
...
def format_offscreen_lines(state: GameState) -> list[str]:
    ...
    for cid, entry in state.characters_offscreen.items():
        ...
        if len(parts) == 1:
            ...
            lines.append(f"{cid} — {parts[1]}")
        else:
            lines.append(f"{cid} — {parts[1]}: {parts[2]}")
    return lines
