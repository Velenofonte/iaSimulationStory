...
def list_eras(story_id: str) -> list[dict[str, str]]:
    """List era entry points for a story pack."""
    from app.story.era_loader import EraLoader

    loader = EraLoader(story_id=story_id)
    items: list[dict[str, str]] = []
    for era_id in loader.list_ids():
        try:
        ...
        items.append(
            {
                "id": era.id,
                "name": era.name,
                "description": era.covers_backstory or era.canon_source,
                "entry_arc": era.entry_arc or "",
                "start_location": era.start_location,
            }
        )
    return items


def load_era(story_id: str, era_id: str):
    """Load a validated EraDefinition for story_id."""
    from app.story.era_loader import EraLoader

    return EraLoader(story_id=story_id).load(era_id)
