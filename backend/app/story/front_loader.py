...
@dataclass
class ArcCloseEffect:
    world_flags: dict[str, bool] = field(default_factory=dict)
    wiki_writes: list[dict[str, Any]] = field(default_factory=list)
...
class FrontDefinition:
    ...
    player_role: str = "external"
    playable_by: list[str] = field(default_factory=list)
    on_close: dict[str, ArcCloseEffect] = field(default_factory=dict)
    start_relative_to: str | None = None
    stall_after_days: int | None = None
    overlays_inherits: str | None = None
...
def _parse_beat(raw: dict[str, Any]) -> FrontBeat:
    ...
    return FrontBeat(
        ...
        cast_acting=_as_str_list(raw.get("cast_acting")),
        ambient=_as_str_list(raw.get("ambient")),
...
def parse_front_dict(data: dict[str, Any]) -> FrontDefinition:
    ...
    overlays_inherits = None
    if isinstance(overlays, dict):
        temporal = list(dict.fromkeys(temporal + _as_str_list(overlays.get("notes"))))
        inherits = overlays.get("inherits")
        if inherits:
            overlays_inherits = str(inherits).strip() or None

    on_close: dict[str, ArcCloseEffect] = {}
    raw_on_close = data.get("on_close") or {}
    if isinstance(raw_on_close, dict):
        for key, raw_effect in raw_on_close.items():
            if not isinstance(raw_effect, dict):
                continue
            wiki_writes = raw_effect.get("wiki_writes") or []
            if not isinstance(wiki_writes, list):
                wiki_writes = []
            chronicle = raw_effect.get("chronicle") or []
            if not isinstance(chronicle, list):
                chronicle = []
            on_close[str(key).strip().lower()] = ArcCloseEffect(
                world_flags={
                    str(k): bool(v)
                    for k, v in (raw_effect.get("world_flags") or {}).items()
                },
                wiki_writes=wiki_writes,
                chronicle=[c for c in chronicle if isinstance(c, dict)],
            )

    relative = start.get("relative_to")
    stall = data.get("stall_after_days")
    playable = _as_str_list(data.get("playable_by"))
    ...
    return FrontDefinition(
        ...
        playable_by=playable,
        on_close=on_close,
        start_relative_to=str(relative).strip() if relative else None,
        stall_after_days=int(stall) if stall is not None else None,
        overlays_inherits=overlays_inherits,
    )
