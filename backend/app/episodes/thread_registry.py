...
from app.player.places import is_travel_intent
...
def ensure_exit_threads(
    ...
    *,
    ...
    """Leaving a scene (new location slug) with open threads must update situations.

    Travel / along-move on the *same* place id does not auto-defer threads — the PG
    may be pursuing that thread in place.
    """
    del player_action  # kept for call-site compatibility
    loc_changed = bool(
        resolution.location
