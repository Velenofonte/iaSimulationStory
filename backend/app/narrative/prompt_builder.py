...
from app.narrative.token_estimate import estimate_tokens, fit_text
...
# Distant cast: appearance only (wiki aspect; no combat/skills/plans).
_DISTANT_SECTION_ALLOWLIST = frozenset({"Aspetto fisico", "Aspetto"})
...
class PromptBuilder:
    def build_prompt_card(
        ...
        *,
        render: bool = False,
        lens_level: str | None = None,
        visible_role: str | None = None,
        sections_allowlist: frozenset[str] | None = None,
        distant: bool = False,
    ) -> str:
        runtime = runtime or {}
        is_player = str(card.role or "").lower() == "player"
        show_true_name = is_player or lens_level in (None, "named")
        if show_true_name:
            name_lines = [
                f"ID: {card.id}",
            ...
            role_txt = (visible_role or card.role or "figura").strip() or "figura"
            name_lines = [
                f"ID: {card.id}",
                f"Nome visibile al PG: {role_txt}",
                f"Nome vero (NARRATOR_ONLY): {card.name or card.id}",
            ]
        lines = [
            *name_lines,
            f"Tier: {card.tier}",
        ...
        if distant:
            lines.append("distant: non in present, non parla")
        if card.role:
        ...
        allow = sections_allowlist
        if distant and allow is None:
            allow = _DISTANT_SECTION_ALLOWLIST
        ...
        budget = 0 if distant else self._tier_budget(card.tier)
