...
from app.models.game_state import NpcKnowledgeFact, OffscreenCharacter
...
def test_along_move_null_present_is_retried() -> None:
    stub = _StubLLM(
        [
            TurnResolution(
                time=NarrativeTime(bucket="istantanea", minutes=5),
                present=None,
                scene_brief=["corre sul sentiero"],
            ),
            TurnResolution(
                time=NarrativeTime(bucket="istantanea", minutes=5),
                present=[],
                present_leave={
                    "kael": OffscreenCharacter(where="osteria", reason="resta al bancone"),
                },
                scene_brief=["corre solo"],
            ),
        ]
    )
    req = NarrativeRequest(
        ...
        player_action="corro lungo il sentiero fino al bivio",
        stance="action",
    )
    out = TurnResolver(llm=stub).resolve(req)
    assert out.present == []
    assert "kael" in out.present_leave
    assert len(stub.systems) == 2
    assert "present_leave" in stub.systems[1]


def test_along_move_identical_present_without_leave_is_retried() -> None:
    stub = _StubLLM(
        [
            TurnResolution(
                time=NarrativeTime(bucket="istantanea", minutes=5),
                present=["kael", "oste"],
                scene_brief=["si allontana"],
            ),
            TurnResolution(
                time=NarrativeTime(bucket="istantanea", minutes=5),
                present=[],
                scene_brief=["resta solo"],
            ),
        ]
    )
    req = NarrativeRequest(
...
def test_wait_without_progress_is_retried() -> None:
    stub = _StubLLM(
        [
            TurnResolution(
                time=NarrativeTime(bucket="media", minutes=20),
                scene_brief=["il vento soffia, l'attesa continua"],
            ),
            TurnResolution(
                time=NarrativeTime(bucket="media", minutes=20),
                scene_brief=["una figura emerge dalla nebbia"],
                situations_add=["figura in vista"],
            ),
        ]
    )
    req = NarrativeRequest(
        canon_facts=NarrativeCanonFacts(
            ...
            time="Giorno 1 · sera",
            situations=[
                NpcKnowledgeFact(id="threat_arrival", summary="Minaccia in arrivo"),
            ],
        ),
        player_action="aspetto",
        stance="wait",
    ...
    assert out.situations_add
    assert len(stub.systems) == 2
    assert "conseguenza" in stub.systems[1].casefold() or "atmosfera" in stub.systems[1].casefold()


def test_wait_with_progress_is_not_retried() -> None:
    stub = _StubLLM(
        [
            TurnResolution(
                time=NarrativeTime(bucket="media", minutes=15),
                scene_brief=["arriva un messaggero"],
                situations_add=["messaggero"],
            )
        ]
    )
    req = NarrativeRequest(
    ...
    out = TurnResolver(llm=stub).resolve(req)
    assert [s.summary for s in out.situations_add] == ["messaggero"]
    assert len(stub.systems) == 1
