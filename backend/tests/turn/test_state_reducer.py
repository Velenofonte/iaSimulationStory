...
def test_resolution_to_delta_clears_present_on_along_move_when_omitted() -> None:
    res = TurnResolution(
...
def test_resolution_to_delta_keeps_explicit_present_on_along_move() -> None:
    res = TurnResolution(
        time=NarrativeTime(bucket="istantanea", minutes=5),
        location=None,
        present=["kael"],
    ...
    delta = resolution_to_delta(
        res,
        previous_location="sentiero",
        player_action="cammino sul ciglio",
    )
    assert delta.characters_active == ["kael"]


def test_resolution_to_delta_no_along_omitted_present_is_no_touch() -> None:
    res = TurnResolution(
        time=NarrativeTime(bucket="istantanea", minutes=3),
        location=None,
        present=None,
    ...
    delta = resolution_to_delta(
        res,
        previous_location="osteria",
        player_action="guardo intorno",
    )
    assert delta.characters_active is None
...
def test_format_offscreen_lines() -> None:
    ...
    learn(state, "nfirea", "named")
    learn(state, "solo", "named")
    lines = format_offscreen_lines(state)
    assert "nfirea — retrobottega: erbe" in lines
    assert "solo" in lines


def test_format_offscreen_lines_masks_unknown() -> None:
    from app.models.game_state import OffscreenCharacter
    from app.services.state_reducer import format_offscreen_lines
    ...
    state = GameState(
        session_id="s",
        characters_offscreen={
            "pabel": OffscreenCharacter(where="wall", reason="nearby"),
        },
    )
    lines = format_offscreen_lines(state, roles={"pabel": "sergente"})
    assert any("NARRATOR_ONLY" in line for line in lines)
