...
def test_same_slug_travel_does_not_defer_situations() -> None:
    state = _state(
        situations=[
            NpcKnowledgeFact(id="minaccia", summary="Minaccia in arrivo sul sentiero")
        ]
    )
    out = ensure_exit_threads(
        _resolution(location=None),
        player_action="mi dirigo a sud lungo il sentiero",
